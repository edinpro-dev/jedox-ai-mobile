...
const Button: React.FC<ButtonProps> = ({
    ...
    const getVariantClasses = (): { button: string; text: string } => {
        ...
        switch (variant) {
            case "primary":
                return {
                    button: `${baseButtonClasses} ${
                        disabled ? "bg-primary/50 border-primary/50" : "bg-primary border-primary"
                    }`,
            ...
            case "secondary":
                return {
                    button: `${baseButtonClasses} ${
                        disabled ? "bg-secondary/50 border-secondary/50" : "bg-secondary border-secondary"
                    }`,
                    text: `font-poppins-semibold ${disabled ? "text-base-content" : "text-white"}`,
                };
            case "accent":
                return {
                    button: `${baseButtonClasses} ${
                        disabled ? "bg-accent/50 border-accent/50" : "bg-accent border-accent"
                    }`,
                    text: `font-poppins-semibold ${disabled ? "text-base-content" : "text-white"}`,
                };
            case "neutral":
                return {
                    button: `${baseButtonClasses} ${
                        disabled ? "bg-neutral/50 border-neutral/50" : "bg-neutral border-neutral"
                    }`,
                    text: `font-poppins-semibold ${disabled ? "text-base-content" : "text-white"}`,
                };
            case "outline":
    ...
    return (
        <TouchableOpacity className={buttonClasses} onPress={onPress} disabled={disabled} activeOpacity={0.8}>
            {isLoading ? (
